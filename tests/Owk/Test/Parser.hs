...
case_function_0 = parseOwk "<test>" "{ 1 }" @?= Right (Program [Function [(PVariable "_" Nothing, Nothing, [Number (I 1)])]])
case_function_1 = parseOwk "<test>" "{ 1; 2 }" @?= Right (Program [Function [(PVariable "_" Nothing, Nothing, [Number (I 1), Number (I 2)])]])
case_function_2 = parseOwk "<test>" "(x, y) -> { 1 }" @?= Right (Program [Function [(PTuple [PVariable "x" Nothing, PVariable "y" Nothing], Nothing, [Number (I 1)])]])
case_function_3 = parseOwk "<test>" "0 -> { \"zero\" } | 1 -> { \"one\" } | n -> { n }"
                      @?= Right (Program [Function [(PNumber (I 0), Nothing, [String "zero"]), (PNumber (I 1), Nothing, [String "one"]), (PVariable "n" Nothing, Nothing, [Variable "n"])]])
case_function_4 = parseOwk "<test>" "f x y" @?= Right (Program [FuncCall (FuncCall (Variable "f") (Variable "x")) (Variable "y")])
case_function_5 = parseOwk "<test>" "_ -> 1" @?= Right (Program [Function [(PVariable "_" Nothing, Nothing, [Number (I 1)])]])
case_function_6 = parseOwk "<test>" "x -> print x" @?= Right (Program [Function [(PVariable "x" Nothing, Nothing, [FuncCall (Variable "print") (Variable "x")])]])
case_function_7 = parseOwk "<test>" "x -> y -> 1" @?= Right (Program [Function [(PVariable "x" Nothing, Nothing, [Function [(PVariable "y" Nothing, Nothing, [Number (I 1)])]])]])

case_function_guard_0 = parseOwk "<test>" "x (x > 10) -> x" @?= Right (Program [Function [(PVariable "x" Nothing, Just (FuncCall (FuncCall (Variable ">") (Variable "x")) (Number (I 10))), [Variable "x"])]])

case_variable_0 = parseOwk "<test>" "x" @?= Right (Program [Variable "x"])

case_define_0 = parseOwk "<test>" "x = 42" @?= Right (Program [Define (PVariable "x" Nothing) (Number (I 42))])
case_define_1 = parseOwk "<test>" "x = { 42 }" @?= Right (Program [Define (PVariable "x" Nothing) (Function [(PVariable "_" Nothing, Nothing, [(Number (I 42))])])])
case_define_2 = parseOwk "<test>" "x = $ -> { 42 }" @?= Right (Program [Define (PVariable "x" Nothing) (Function [(PVariable "$" Nothing, Nothing, [(Number (I 42))])])])
case_define_3 = parseOwk "<test>" "[x, y] = [1, 2]" @?= Right (Program [Define (PList [PVariable "x" Nothing, PVariable "y" Nothing]) (List [Number (I 1), Number (I 2)])])
case_define_4 = parseOwk "<test>" "[1, y] = [1, 2]" @?= Right (Program [Define (PList [PNumber (I 1), PVariable "y" Nothing]) (List [Number (I 1), Number (I 2)])])
...
case_define_7 = parseOwk "<test>" "x@[1, y@2, {k=>z@3}] = a" @?= Right (Program [Define (PVariable "x" (Just (PList [PNumber (I 1), PVariable "y" (Just (PNumber (I 2))), PDict [("k", PVariable "z" (Just (PNumber (I 3))))]]))) (Variable "a")])
