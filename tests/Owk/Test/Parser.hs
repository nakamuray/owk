...
case_number_2 = parseOwk "<test>" "-100" @?= Right (Program [FuncCall (Variable "__neg__") (Number 100)])
...
case_function_guard_0 = parseOwk "<test>" "x (x > 10) -> x" @?= Right (Program [Function [(PVariable "x" Nothing, Just (FuncCall (FuncCall (Variable ">") (Variable "x")) (Number 10)), [Variable "x"])]])
...
case_define_2 = parseOwk "<test>" "x = $ -> { 42 }" @?= Right (Program [Define (PVariable "x" Nothing) (Function [(PVariable "$" Nothing, Nothing, [(Number 42)])])])
...
case_operator_0 = parseOwk "<test>" "1 + 2" @?= Right (Program [FuncCall (FuncCall (Variable "+") (Number 1)) (Number 2)])
case_operator_1 = parseOwk "<test>" "`+` 1 2" @?= Right (Program [FuncCall (FuncCall (Variable "+") (Number 1)) (Number 2)])
case_opratorr_2 = parseOwk "<test>" "`+:` = undef" @?= Right (Program [Define (PVariable "+:" Nothing) (Variable "undef")])
case_opratorr_3 = parseOwk "<test>" "1 +: 2 *: 3" @?= Right (Program [FuncCall (FuncCall (Variable "+:") (Number 1)) (FuncCall (FuncCall (Variable "*:") (Number 2)) (Number 3))])
case_opratorr_4 = parseOwk "<test>" "1 *: 2 +: 3" @?= Right (Program [FuncCall (FuncCall (Variable "+:") (FuncCall (FuncCall (Variable "*:") (Number 1)) (Number 2))) (Number 3)])
...
case_newline_15 = parseOwk "<test>" "1 + \n2\n3" @?= Right (Program [FuncCall (FuncCall (Variable "+") (Number 1)) (Number 2), Number 3])
